...
const BagForm2 = () => {
  ...
  useEffect(() => {
    ...
    // Disconnect socket when the component unmounts (user closes the site or leaves the page)
    return () => {
      if (socketServer) {
  ...
  const handleEditOrder = (order) => {
    ...
    setSelectedColor(
      colours.find((color) => color.colourName === order.o_colour)?.colourCode
    );
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (data?.success) {
        ...
        setOrderData({
          ...orderData,
          o_category: "Non Woven Bag", // Set the default selected category
        });
      }
    } catch (error) {
      console.log(error);
  ...
  const handleColorChange = (event) => {
    ...
    const selectedColorObj = colours.find(
      (color) => color.colourCode === selectedValue
    );
    if (selectedColorObj) {
      setSelectedColorName(selectedColorObj.colourName);
      setOrderData({ ...orderData, o_colour: selectedColorObj.colourName });
    } else {
      setSelectedColorName("");
  ...
  const handleOk = async () => {
    try {
      ...
      // Send the order data to create-bag-order API
      await axios.post(
        `${process.env.REACT_APP_API}/api/v1/bag-order/create-bag-order`,
        { orders }
      );
        ...
        <form>
          ...
          <div className="mb-3">
            ...
            >
              ...
              {colours.map((colour) => (
                <option key={colour._id} value={colour.colourCode}>
                  {colour.colourName}
                </option>
              ))}
            </select>
